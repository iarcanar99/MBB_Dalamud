...
class TranslatorGemini:
    ...
    def get_relevant_names(self, text):
        ...
        essential_names = {"Y'shtola", "Alphinaud", "Alisaie", "Wuk Lamat", "???"}
        for name in essential_names:
            if name in self.character_names_cache:
                relevant_names.add(name)

        # Limit to maximum 10 names to control token usage
        return list(relevant_names)[:10]
    ...
    def get_rpg_general_prompt(self):
        ...
        return (
            ...
            "3. **Name Preservation**: NEVER translate character names, place names, or special terms from the database\n"
